---
name: add-endpoint
description: Use when the user asks to add a REST API endpoint, expose backend data to the frontend, or create a new page/tab that needs a new fetch path.
---
...
如果该数据有独立的 DB 表和同步函数，在 `data_health.py` 的 `CHECKS` 数组添加 `CheckDef`。
