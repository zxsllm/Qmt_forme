...
- **细分主线硬下限：当日同细分 ≥ 4 只涨停**。少于 4 只一律按补涨 / 杂毛处理，**即使**有 ≥ 2 连板高标也不立主线（孤独高标 = 个股事件，不是板块抱团）
- ≤ 3 只涨停的"主线候选"对下游回测的板块共识判定毫无价值（容易触发虚假信号），必须剔除
- **跳过基本面属性标签**："一季报预增" / "年报增长" / "业绩扭亏" / "净利润增长" 都不是题材主线
- 仅同板块涨停 1-3 只的"地产" / "造纸" / "汽车配件"这种纯行业大标签，按补涨处理
...
注：5/7 当日液冷只有 2 只涨停（大中转债 / 欧通转债），低于 ≥4 硬下限，**不立主线**——即使是"算力大筐"的细分，也不享受豁免。

**关键观察**：
- 上面 8 个细分主线里，光通信 / PCB / 算力租赁 / 数据中心 都属于"算力大筐"，但它们是 4 个独立主线（不同启动股、不同跟风逻辑），**不要合并**
