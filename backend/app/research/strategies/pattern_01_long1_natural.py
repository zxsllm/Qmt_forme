            ...
            if lh.shadow:
                ...
                if cb_code:
                    sec_limit_n, sec_broken_n = await count_sector_limit_state(
                        session, trade_date, codes, shadow.first_time
                    )
                    upgrade_overnight = (
                    ...
                    logger.info(
                        "pattern_01 funnel %s sector=%s L_CB shadow_ft=%s "
                        "limits=%d/%d broken=%d/%d decision=%s",
                        trade_date, sec_name, shadow.first_time,
                        sec_limit_n, L_CB_OVERNIGHT_LIMIT_MIN,
                    ...
                    if upgrade_overnight:
                        ...
                        cb_tag = "[L_CB 影子龙债 升级隔夜]"
                    else:
                        sell_anchor_kw = {
                            "sell_anchor": "intraday_at",
                            "sell_anchor_time": shadow.first_time,
                        }
                        cb_tag = "[L_CB 影子龙债 T+0 影子龙封板出]"
