...
async def test_rich_stock_single_batch_match(session):
    single = await score_stock(session, RICH_STOCK, TRADE_DATE)
    ctx = await prefetch_batch(session, TRADE_DATE, [RICH_STOCK])
    batch = await score_stock(session, RICH_STOCK, TRADE_DATE, ctx=ctx)
...
async def test_sparse_stock_single_batch_match(session):
    single = await score_stock(session, SPARSE_STOCK, TRADE_DATE)
    ctx = await prefetch_batch(session, TRADE_DATE, [SPARSE_STOCK])
    batch = await score_stock(session, SPARSE_STOCK, TRADE_DATE, ctx=ctx)
    for key in ("total_score", "tech_score", "sentiment_score",
                "fundamental_score", "news_score"):
        assert single[key] == batch[key], f"{key}: {single[key]} != {batch[key]}"


async def test_ranked_contains_consistent_score(session):
    ranked = await rank_stocks(session, TRADE_DATE, limit=2000, min_score=0)
...
async def test_fina_single_respects_ann_date(session):
    inputs = await fetch_fundamental_inputs(session, FUTURE_PROBE_STOCK, FUTURE_PROBE_DATE)
    fina_row = inputs["fina_row"]
    if fina_row is not None:
        assert fina_row[0] != "20251231", (
            f"Future fina leak: end_date=20251231 (ann_date=20260414) "
            f"visible at trade_date={FUTURE_PROBE_DATE}"
        )


async def test_fina_batch_respects_ann_date(session):
    ctx = await prefetch_batch(session, FUTURE_PROBE_DATE, [FUTURE_PROBE_STOCK])
    fina_row = ctx["fina_data"].get(FUTURE_PROBE_STOCK)
    if fina_row is not None:
        assert fina_row[0] != "20251231", (
            f"Future fina leak (batch): end_date=20251231 "
            f"visible at trade_date={FUTURE_PROBE_DATE}"
        )


async def test_bars_no_future_dates(session):
    inputs = await fetch_tech_inputs(session, RICH_STOCK, FUTURE_PROBE_DATE)
    bars = inputs["bars"]
    if bars:
        assert bars[0][0] <= FUTURE_PROBE_DATE, f"Future bar: {bars[0][0]}"


async def test_anns_no_future_dates(session):
...
async def test_sparse_stock_no_crash(session):
    result = await score_stock(session, SPARSE_STOCK, TRADE_DATE)
    for key in ("total_score", "tech_score", "sentiment_score",
                "fundamental_score", "news_score"):
...
async def test_sparse_sentiment_no_lhb(session):
    inputs = await fetch_sentiment_inputs(session, SPARSE_STOCK, TRADE_DATE)
...
async def test_sparse_news_zero_counts(session):
    inputs = await fetch_news_inputs(session, SPARSE_STOCK, TRADE_DATE)
...
async def test_batch_mixed_stocks(session):
    ctx = await prefetch_batch(session, TRADE_DATE, [RICH_STOCK, SPARSE_STOCK])
    for code in (RICH_STOCK, SPARSE_STOCK):
        result = await score_stock(session, code, TRADE_DATE, ctx=ctx)
